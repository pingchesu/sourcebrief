...
def seal_receipt(receipt: dict) -> dict:
    receipt["receipt_sha256"] = sha256_digest(
        {key: value for key, value in receipt.items() if key != "receipt_sha256"}
    )
    return receipt


def valid_envelope(*, direct_successes: int = 6, ai_successes: int = 9) -> dict:
...
def test_internal_outcome_pass_is_non_promotional_and_baseline_relative() -> None:
    summary = validate_internal_outcome(valid_envelope(), key=KEY)
...
def test_internal_outcome_strong_direct_baseline_forces_fail_and_stop() -> None:
    summary = validate_internal_outcome(valid_envelope(direct_successes=8), key=KEY)
...
def test_internal_outcome_rejects_missing_direct_coverage_authority_lies_and_tamper() -> None:
    ...
    with pytest.raises(EvalManifestError, match="5x12 coverage"):
        validate_internal_outcome(missing, key=KEY)

    authority = valid_envelope()
    ...
    with pytest.raises(EvalManifestError, match="never be D0"):
        validate_internal_outcome(authority, key=KEY)

    tampered = valid_envelope()
    tampered["run_finished_at"] = "2026-07-21T10:11:00Z"
    with pytest.raises(EvalManifestError, match="MAC"):
        validate_internal_outcome(tampered, key=KEY)


def test_internal_outcome_rejects_split_runtime_and_receipt_scalar_bypasses() -> None:
    bad_id = valid_envelope()
    ...
    with pytest.raises(EvalManifestError, match="opaque"):
        validate_internal_outcome(bad_id, key=KEY)

    bool_budget = valid_envelope()

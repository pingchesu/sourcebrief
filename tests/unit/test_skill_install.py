...
def _package_manifest(status: str = "approved") -> dict:
    return {
...
def write_package(root: Path, *, status: str = "approved") -> Path:
    ...
    (root / "manifest.json").write_text(json.dumps(_package_manifest(status)) + "\n", encoding="utf-8")
...
def test_skill_install_apply_and_uninstall_with_receipt(tmp_path: Path) -> None:
    ...
    assert receipt_body["package_hash"] == _package_manifest()["package_hash"]

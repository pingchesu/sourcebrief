...
def test_empty_resource_allowlist_cannot_expand_to_project_scope(tmp_path) -> None:
    ...
    mcp_context = client.post(
        ...
        json={
            ...
            "id": 77,
            "method": "tools/call",
            "params": {"name": "sourcebrief.get_agent_context", "arguments": {"query": "checkoutrepo42"}},
        },
        headers=empty_scoped,
    )
...
def test_remote_code_ignores_non_git_resources_and_mcp_returns_tool_errors(tmp_path) -> None:
    ...
    mcp_bad = client.post(
        f"/mcp/{workspace_id}/{project_id}",
        json={"jsonrpc": "2.0", "id": 10, "method": "tools/call", "params": {"name": "sourcebrief.read_file", "arguments": {"resource_id": doc.json()["id"], "path": "/etc/passwd"}}},
        headers=headers,
    )
...
def test_generate_patch_is_opt_in_scoped_and_records_branch_freshness(tmp_path) -> None:
    ...
    generated = client.post(
        ...
        json={
            "resource_id": resource_id,
            "scope": "change checkout marker",
            "source_branch": "feat/sourcebrief-patch",
            "target_branch": "main",
...
def test_open_pr_requires_opt_in_approval_and_rejects_moved_patch(tmp_path) -> None:
    ...
    disabled = client.post(
        ...
        json={"patch_proposal_id": proposal_id, "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "approved in test"},
        headers=pr_token,
    )
    assert disabled.status_code == 403
    ...
    opened = client.post(
        f"/workspaces/{workspace_id}/projects/{project_id}/remote-code/open_pr",
        json={"patch_proposal_id": proposal_id, "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "approved in test", "github_pr_url": "https://github.com/example/repo/pull/1"},
        headers=pr_token,
    )
    assert opened.status_code == 200, opened.text
    assert opened.json()["status"] == "opened"
    assert opened.json()["source_branch"] == "feat/sourcebrief-patch"
    assert opened.json()["target_branch"] == "main"
    assert opened.json()["diff_summary"] == "1 file(s): src/checkout.py"

    duplicate = client.post(
        f"/workspaces/{workspace_id}/projects/{project_id}/remote-code/open_pr",
        json={"patch_proposal_id": proposal_id, "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "approved twice"},
        headers=pr_token,
    )
    ...
    blocked = client.post(
        f"/workspaces/{workspace_id}/projects/{project_id}/remote-code/open_pr",
        json={"patch_proposal_id": moved.json()["id"], "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "approved in test"},
        headers=pr_token,
    )
    ...
    missing_base_blocked = client.post(
        f"/workspaces/{workspace_id}/projects/{project_id}/remote-code/open_pr",
        json={"patch_proposal_id": missing_base.json()["id"], "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "missing base"},
        headers=pr_token,
    )
    ...
    stale_approval = client.post(
        f"/workspaces/{workspace_id}/projects/{project_id}/remote-code/open_pr",
        json={"patch_proposal_id": fresh.json()["id"], "source_branch": "feat/sourcebrief-patch", "target_branch": "main", "approval_note": "stale after refresh"},
        headers=pr_token,
    )
...
def test_pr_write_resource_scope_and_invalid_patch_paths_fail_closed(tmp_path) -> None:
    ...
    mcp_invalid = client.post(
        ...
        json={
            ...
            "id": 9,
            "method": "tools/call",
            "params": {"name": "sourcebrief.generate_patch", "arguments": {"resource_id": first_resource, "scope": "invalid path", "files": [{"path": "/etc/passwd", "start_line": 1, "end_line": 1, "new_content": "x"}]}},
        },
        headers=first_patch_token,
    )
...
def test_mcp_patch_tools_are_errors_until_policy_and_scopes_allow(tmp_path) -> None:
    ...
    assert {"sourcebrief.generate_patch", "sourcebrief.open_pr"}.issubset(names)

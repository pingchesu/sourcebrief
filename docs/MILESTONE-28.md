...
- Evaluate git repo-agent health from existing SourceBrief evidence:
...
The scheduler is the only default persistent path and writes only SourceBrief summary/audit records. It does not rotate tokens, enable write tools, mutate source repos, send Slack/webhook messages, publish skill packs, or open PRs.
...
`SOURCEBRIEF_AGENT_CARD_AUDITOR=true` enables maintenance-time audits. `SOURCEBRIEF_AGENT_CARD_AUDIT_INTERVAL_HOURS` defaults to `24`.
...
- `DATABASE_URL=postgresql+psycopg://sourcebrief:sourcebrief@localhost:55432/sourcebrief .venv/bin/alembic upgrade head` passed.

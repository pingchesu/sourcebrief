"""Resource ingestion for SourceBrief Milestone 2.
...
from sourcebrief_worker.section_store import build_snapshot_sections
...
def _request_for_url(url: str) -> Request:
    return Request(
        url,
        headers={
            "User-Agent": "SourceBriefBot/0.1 (+https://github.com/pingchesu/sourcebrief)",
...
def _work_base() -> str:
    base = os.getenv("SOURCEBRIEF_WORK_DIR", os.getenv("CONTEXTSMITH_WORK_DIR")) or os.path.join(
        tempfile.gettempdir(), "sourcebrief-ingest"
    )
...
def _collect_folder_bundle(resource: Resource) -> tuple[list[dict], str, str, dict]:
    from sourcebrief_worker.bundle_ingest import assert_under_uploads, extract_zip_to_sandbox

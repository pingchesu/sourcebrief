...
def main() -> None:
    ...
    interval_seconds = int(os.getenv("SOURCEBRIEF_MAINTENANCE_INTERVAL_SECONDS", os.getenv("CONTEXTSMITH_MAINTENANCE_INTERVAL_SECONDS", "60")))
    limit = int(os.getenv("SOURCEBRIEF_MAINTENANCE_LIMIT", os.getenv("CONTEXTSMITH_MAINTENANCE_LIMIT", "100")))
    audit_interval_hours = int(os.getenv("SOURCEBRIEF_AGENT_CARD_AUDIT_INTERVAL_HOURS", os.getenv("CONTEXTSMITH_AGENT_CARD_AUDIT_INTERVAL_HOURS", "24")))
    enable_agent_card_audits = os.getenv("SOURCEBRIEF_AGENT_CARD_AUDITOR", os.getenv("CONTEXTSMITH_AGENT_CARD_AUDITOR", "true")).lower() == "true"
    print("SourceBrief maintenance scheduler started", flush=True)
    while running:
        try:
        ...
        for _ in range(max(1, interval_seconds)):
            if not running:
                break
            time.sleep(1)
    print("SourceBrief maintenance scheduler stopped", flush=True)

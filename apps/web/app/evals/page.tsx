...
import type { RetrievalEvalQuestion, RetrievalEvalResponse, RetrievalEvalRunList, RetrievalProfilesResponse } from '../../lib/types';
...
export default function QualityEvalsPage() {
  ...
  const activeProfile = profileCatalog?.profiles.find((profile) => profile.name === selectedProfile);
  ...
  useEffect(() => { void loadHistory(); void loadProfiles(); }, [settings.workspaceId, settings.projectId]);
  ...
  return <main className="page">
    <PageHeader eyebrow="Quality Evals" title="Retrieval quality gate" description="Run and persist project-level golden questions against the real agent-context path. Compare lexical/vector/hybrid/rerank/graph profiles with evidence instead of one-off guesses." actions={<button className="btn secondary" disabled={!selected} onClick={seedQuestions}>Seed from selected repo</button>} />
    ...
    <div className="grid two">
      <Card>
        ...
        {indexedResources.length === 0 ? <EmptyState text="Index at least one resource before running evals." /> : <form className="grid" onSubmit={runEval}>
          ...
          <Field label="Retrieval profile"><select className="input" value={selectedProfile} onChange={(event) => setSelectedProfile(event.target.value)}>{(profileCatalog?.profiles ?? []).map((profile) => <option key={profile.name} value={profile.name}>{profile.name}</option>)}</select></Field>
          ...
          <button className="btn" disabled={busy || !questionsJson.trim()}>{busy ? 'Running...' : `Run ${selectedProfile} eval`}</button>
        </form>}
      ...
      <Card>
        ...
        {!result ? <EmptyState text="No eval result selected yet. Run an eval or load a historical run." /> : <div className="grid">
          <div className="grid four"><Metric label="Status" value={result.summary.status} /><Metric label="Profile" value={result.profile} /><Metric label="Pass rate" value={`${Math.round(result.summary.pass_rate * 100)}%`} /><Metric label="Avg latency" value={`${result.summary.avg_latency_ms}ms`} /></div>

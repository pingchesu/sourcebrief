  ...
  return <main className="page">
    ...
    <div className="grid two">
      <SectionCard title="Compile merge draft" description="Select two or more current resource graph versions. E1 rejects multi-version same-resource merges.">
        <form onSubmit={compile} className="grid">
          ...
          <Field label="Published source graphs"><div className="graph-source-list">{publishedGraphs.map((graph) => <label key={graph.graph_key} className="graph-source-row"><input type="checkbox" checked={selectedInputs.includes(graph.graph_key)} onChange={(event) => setSelectedInputs((prev) => event.target.checked ? [...prev, graph.graph_key] : prev.filter((key) => key !== graph.graph_key))} /><span><strong>{graph.title}</strong><span className="muted">{graph.graph_key} · v{graph.current?.version}</span></span></label>)}</div></Field>
          <button className="btn primary" disabled={busy || selectedInputs.length < 2}>Compile draft</button>
        </form>
      </SectionCard>
      <SectionCard title="Merge streams">
        {merges.length ? <div className="grid">{merges.map((merge) => <button key={merge.merge_key} className={`list-row ${selected?.merge_key === merge.merge_key ? 'active' : ''}`} onClick={() => setSelectedKey(merge.merge_key)}><span><strong>{merge.title}</strong><span className="muted">{merge.merge_key}</span></span><StatusChip value={merge.current ? `current v${merge.current.version}` : merge.status} /></button>)}</div> : <EmptyState text="No graph merge yet. Select at least two published graphs and compile a draft." />}
      </SectionCard>
    </div>

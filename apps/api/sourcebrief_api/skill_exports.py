...
def _render_skill(version: ContextPackVersion, title: str, summary: str | None, resources: list[str], counts: dict[str, int]) -> str:
    ...
    description = summary or f"Use SourceBrief published pack {version.pack_key} v{version.version} for cited runtime context."
    return f"""---
name: {name}
...
# SourceBrief runtime skill: {title}

Use this skill when a user asks about knowledge covered by SourceBrief Context Pack `{version.pack_key}` version `{version.version}`.
...
- SourceBrief is the source of truth. Request context at runtime and cite returned evidence.
...
1. Call SourceBrief with the user query and the pinned pack selector:
...
1. `sourcebrief.get_context_pack` with `pack_key` / `version` to inspect the pinned pack, freshness, sources, and graph inventory.
2. `sourcebrief.search` with `context_pack_key` / `context_pack_version` to find cited section evidence.
3. `sourcebrief.read_section` using the canonical locator returned by `search`, `get_context_pack`, or `get_resource_map` before making source-specific claims.
4. `sourcebrief.get_graph_inventory`, `sourcebrief.graph_query`, or `sourcebrief.graph_path` for architecture and impact questions.
5. Fall back to stable `sourcebrief.get_agent_context` with the same selector when expanded MCP tools are not advertised. Use `search_code`, `grep_code`, `read_file`, and `find_symbol` only for exact repo file/symbol evidence.
...
3. Answer only from returned evidence. Cite paths/sections from SourceBrief citations. If evidence is insufficient, say so and request more context.
4. If SourceBrief is unavailable, the pack is invalidated, or the returned pack metadata does not match this file, do not guess. Report degraded context.

## Freshness and mutation boundary

- Treat SourceBrief freshness warnings as blocking for production-sensitive claims.
...
- Auth denied: ask the user to grant SourceBrief access; do not ask for raw bearer tokens in chat.
- Missing MCP: use REST/API fallback if configured, otherwise ask the user to configure SourceBrief runtime access.
...
This is a generated SourceBrief runtime adapter for Context Pack `{version.pack_key}` v`{version.version}`.

Status: `{status}`. External install/copy is allowed only after the export is `approved` in SourceBrief.

The package contains no source corpus. It requires SourceBrief runtime access and uses the pinned `context_pack_key` + `context_pack_version` selector.
...
def _validate_files(files: list[dict[str, Any]], skill_content: str) -> dict[str, Any]:
    ...
    required_text = ["context_pack_key", "context_pack_version", "context_pack_snapshot_pin_enforced", "sourcebrief.get_agent_context", "citations", "mutation boundary"]
